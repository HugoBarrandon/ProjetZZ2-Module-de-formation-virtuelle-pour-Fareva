...
public class Screw : MonoBehaviour
{
    ...
    public void onEdgeEnter(string edgeName, Collider collider)
    {
        ...
        if (edgeName == "GlobalEdge" && collider.tag == "Screwable" && isPlanted)
        {
            if(mainPiece == null)
            {
                GameObject newParent = collider.gameObject;
                if (collider.gameObject.layer != 16) //on va chercher le parent le plus proche de type module
                {
            ...
            else if(collider.gameObject != mainPiece)
            {
                ...
                newParent.transform.SetParent(mainPiece.transform);
                Destroy(newParent.GetComponent<XRGrabInteractable>());
                Destroy(newParent.GetComponent<Rigidbody>());
